...
Knob and exposes its selected climate and light entities in the same JSON
attribute format as the manual Template-Entity configuration.
...
name, and select its climate and light entities. Repeat **Add integration** for
every additional Smart Knob.
...
At least one climate entity is required. Light entities are optional. The
screen order follows the selector order, and display names come from the Home
Assistant friendly names. To change a displayed name, rename the entity and
